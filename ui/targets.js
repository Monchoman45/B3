...
B3.ui.targets.category = {
	...
	prepare: function(params) {
		params.category = params.category.charAt(0).toUpperCase() + params.category.substring(1);
		return params;
	},
	label: function(params) {
		if(params.category.toLowerCase().indexOf('category:') != 0) {params.category = 'Category:' + params.category[0].toUpperCase() + params.category.substring(1);}
		else {params.category = 'Category:' + params.category['Category:'.length].toUpperCase() + params.category.substring('Category:'.length + 1);}

		var text = document.createTextNode('All pages in ');
		var a = document.createElement('a');
			var url = encodeURIComponent('Category:' + params.category);
			while(url.indexOf('%20') != -1) {url = url.replace('%20', '_');}
			while(url.indexOf('%2F') != -1) {url = url.replace('%2F', '/');}
			while(url.indexOf('%3A') != -1) {url = url.replace('%3A', ':');}
			a.href = '/wiki/' + url;
	...
	fetch: function(params, success, failure) {
		B3.api.categorymembers(params.category, 5000, function(titles) {
			if(typeof success == 'function') {
				for(var i = 0; i < titles.length; i++) {titles[i] = titles[i].title;}
				success.call(this, titles);
			}
		}, failure);
	}
};
